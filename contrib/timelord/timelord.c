...
void goaway(int signal)
{
	if (nbp_unrgstr(server, "TimeLord", "*", NULL) < 0) {
		LOG(log_error, logtype_default, "Can't unregister %s",
		    server);
		exit(1);
	}
...
int main(int ac, char **av)
{
	...
	if ((atp = atp_open(ATADDR_ANYPORT, &sat.sat_addr)) == NULL) {
		LOG(log_error, logtype_default, "main: atp_open: %s",
		    strerror(errno));
		exit(1);
	}

	if (nbp_rgstr(atp_sockaddr(atp), server, "TimeLord", "*") < 0) {
		LOG(log_error, logtype_default, "Can't register %s",
		    server);
		exit(1);
	}
	...
	for (;;) {
		...
		if (atp_rreq(atp, &atpb) < 0) {
			LOG(log_error, logtype_default,
			    "main: atp_rreq: %s", strerror(errno));
			exit(1);
		}
		...
		case TL_GETTIME:
			...
			bcopy(&mtime, buf + SIZEOF_MAC_LONG,
			      SIZEOF_MAC_LONG);
		...
		if (atp_sresp(atp, &atpb) < 0) {
			LOG(log_error, logtype_default,
			    "main: atp_sresp: %s", strerror(errno));
			exit(1);
		}
	}
}
